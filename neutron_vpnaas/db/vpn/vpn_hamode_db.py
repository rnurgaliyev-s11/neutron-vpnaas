...
VPN_HA_OPTS = [
    cfg.BoolOpt('vpn_ha',
                default=False,
                help=_('Enable HA mode for VPN virtual routers.')),
...
class VPNRouterHAStatus(model_base.BASEV2):
    """Represent VPN router HA state .
    """
    __tablename__ = 'VPN_router_ha_status'
...
class VPN_HA_Router_db_mixin(router_az_db.RouterAvailabilityZoneMixin):
    """Mixin class to add high availabROUTERility capability to routers."""
    ...
    def __init__(self):
        ...
        super(VPN_HA_Router_db_mixin, self).__init__()

    def vpn_router_is_ha(self, context, router_id):
        ha = (context.session.query(VPNRouterHAStatus).
         filter(VPNRouterHAStatus.router_id == router_id).
         first())
        if ha == None:
            return False
        return ha

    def vpn_router_set_ha(self, context, router_id, ha):
        with context.session.begin():
            vpnrouterhastatus = VPNRouterHAStatus(
                router_id=router_id, ha=ha)
            context.session.add(vpnrouterhastatus)

    def vpn_router_clear_ha(self, context, router_id):
        with context.session.begin(subtransactions=True):
            context.session.query(VPNRouterHAStatus).filter_by(
        ...
        for count in range(MAX_ALLOCATION_TRIES):
            try:
                ...
                with context.session.begin():
                    ...
                    #TODO bypass add the data to DB for OVN+VPN
    ...
    def _set_vr_id(self, context, router, ha_network):
        self._allocate_vr_id(context, ha_network.network_id, router.id)
    ...
    def _create_ha_network(self, context, tenant_id):
        ...
        ha_network = self.l3_plugin._admin_net._get_l3_admin_net(create=True)
    ...
    def _create_ha_port_binding(self, context, router_id, port_id):
        try:
            with context.session.begin(subtransactions=True):
                portbinding = VPNHARouterAgentPortBinding()
                portbinding.port_id = port_id
                portbinding.router_id = router_id
    ...
    def add_ha_port(self, context, router_id, network_id, tenant_id):
        ...
        if context.session.is_active:
            raise RuntimeError(_('add_ha_port cannot be called inside of a '
                                 'transaction.'))
        creation = functools.partial(
            self.ipsec_driver._IPsecHelper._join_transit_and_agent,
            context, router_id)
        content = functools.partial(self._create_ha_port_binding, context,
                                    router_id)
        deletion = functools.partial(
            self.ipsec_driver._IPsecHelper._join_transit_and_agent, context)
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                for port_id in port_ids:
                    self.ipsec_driver._IPsecHelper._disjoin_transit_and_agent(
                        context, router.id, port_id)

    def _delete_ha_interfaces(self, context, router_id):
        with context.session.begin(subtransactions=True):
            context.session.query(VPNHARouterAgentPortBinding).filter_by(
                router_id=router_id).delete()
        self.ipsec_driver._IPsecHelper._disjoin_transit_and_agents(
            context, router_id)
    ...
    def delete_ha_interfaces_on_host(self, context, router_id, host):
        ...
        for port_id in port_ids:
            with context.session.begin(subtransactions=True):
                context.session.query(VPNHARouterAgentPortBinding).filter_by(
                    router_id=router_id, port_id=port_id).delete()
            try:
                self.ipsec_driver._IPsecHelper._disjoin_transit_and_agent(
                    context, router_id, port_id)
            except:
                pass

    def _notify_ha_interfaces_updated(self, context, router_id,
                                      schedule_routers=True):
        self.ipsec_driver.agent_rpc.vpn_routers_updated(
    ...
    def _get_device_owner(self, context, router=None):
        ...
        return super(VPN_HA_Router_db_mixin,
    ...
    def create_vpn_ha_router(self, context, router_id):
        router_db = self.l3_plugin._get_router(context, router_id)
        is_ha = self._is_ha(router_db)
        if is_ha:
            try:
                self.vpn_router_set_ha(context, router_id, is_ha)
                ...
                with excutils.save_and_reraise_exception():
                    self.delete_vpn_ha_router(context, router_id)
        return is_ha
    ...
    def _update_router_db(self, context, router_id, data):
        ...
        with context.session.begin(subtransactions=True):
            router_db = super(VPN_HA_Router_db_mixin, self)._update_router_db(
    ...
    def _delete_ha_network(self, context, net, tenant_id):
        with context.session.begin(subtransactions=True):
            context.session.query(VPNHARouterNetwork).filter_by(
                tenant_id=tenant_id, network_id=net.network_id).delete()

    def safe_delete_ha_network(self, context, ha_network, tenant_id):
        try:
            self._delete_ha_network(context, ha_network, tenant_id)
    ...
    def delete_vpn_ha_router(self, context, id):
        router_db = self.l3_plugin._get_router(context, id)
        is_ha = self.vpn_router_is_ha(context, id)
        if is_ha:
            self._unbind_ha_router(context, id)
            ha_network = self.get_ha_network(context,
                                             router_db.tenant_id)
            if ha_network:
                #self._delete_vr_id_allocation(
                ...
                self.safe_delete_ha_network(context, ha_network,
                                            router_db.tenant_id)
        self.vpn_router_clear_ha(context, id)
    ...
    def get_ha_sync_data_for_host(self, context, host, agent,
        ...
        if (dvr_agent_mode and n_utils.is_extension_supported(
            ...
            sync_data = super(VPN_HA_Router_db_mixin, self).get_sync_data(context,
                                                            router_ids, active)
